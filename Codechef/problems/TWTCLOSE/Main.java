...
class Main {
    ...
    public static void main( String args[] ) throws IOException {
        ...
        int numTweets = ir.readInt();
        int numTestCases = ir.readInt();
        
        boolean[] tweets = new boolean[numTweets];
        
        while( numTestCases > 0 ) {
            String s = ir.readString();
            if( s.equals( "CLICK" ) ) {
                int tweet = ir.readInt();
                tweets[tweet-1] = !tweets[tweet-1];
            }
            else {
                tweets = new boolean[numTweets];
            }
            
            int sumOpenTweets = 0;
            for( boolean b : tweets ) {
                if( b ) sumOpenTweets++;
            }
            ow.println( sumOpenTweets );
